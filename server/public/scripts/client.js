    ...
    itemDiv.innerHTML = `
      ...
      item.queryType
        ...
        <div class="data-details" style="display: none;">
          ...
          <p><strong>Результат:</strong> ${item.output}</p>
        </div>
      `;
...
function getQueryDetailsHTML(item) {
  ...
  switch (item.queryType) {
    case 'sequence':
      detailsHTML = `
          <p><strong>Input:</strong> Кількість - ${item.input.quantity}, Min: ${item.input.min}, Max: ${item.input.max}</p>
        `;
      break;
    case 'random':
      detailsHTML = `
    ...
    case 'randomWord':
      detailsHTML = `
          <p><strong>Input:</strong> Ввід: ${item.input.text}</p>
        `;
      break;
    case 'generatePassword':
      detailsHTML = `
          <p><strong>Input:</strong> Довжина: ${item.input.length}</p>
        `;
      break;
    case 'generatePasswords':
      detailsHTML = `
          <p><strong>Input:</strong> Кількість: ${item.input.quantity}, Довжина: ${item.input.length}</p>
        `;
      break;
    default:
