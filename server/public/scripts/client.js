...
function getQueryHTML(randomQueryData, requestDateString) {
    ...
    if (randomQueryData.queryType === 'sequence') {
      queryHTML = `
        <ul>
          <li>Query Type: Sequence</li>
          <li>Min: ${randomQueryData.input.min}</li>
          <li>Max: ${randomQueryData.input.max}</li>
          <li>Quantity: ${randomQueryData.input.quantity}</li>
          <li>Output: ${randomQueryData.output}</li>
          <li>Request Time: ${requestDateString}</li>
        </ul>
      `;
    } else if (randomQueryData.queryType === 'random') {
      queryHTML = `
        <ul>
          <li>Query Type: Random</li>
          <li>Min: ${randomQueryData.input.min}</li>
          <li>Max: ${randomQueryData.input.max}</li>
        <li>Output: ${randomQueryData.output}</li>
        <li>Request Time: ${requestDateString}</li>
      ...
      queryHTML = `
        <ul>
          <li>Query Type: Random Word</li>
          <li>Text: ${randomQueryData.input.text}</li>
          <li>Output: ${randomQueryData.output}</li>
          <li>Request Time: ${requestDateString}</li>
          </ul>
      `;
      ...
      queryHTML = `
        <ul>
          <li>Query Type: Generate Password</li>
          <li>Length: ${randomQueryData.input.length}</li>
          <li>Output: ${randomQueryData.output}</li>
          <li>Request Time: ${requestDateString}</li>
          </ul>
      `;
      ...
      queryHTML = `
        <ul>
          <li>Query Type: Generate Passwords</li>
          <li>Quantity: ${randomQueryData.input.quantity}</li>
          <li>Length: ${randomQueryData.input.length}</li>
          <li>Output: ${randomQueryData.output.join('; <br><br>')}</li>
          <li>Request Time: ${requestDateString}</li>
          </ul>
      `;
    } else {
  ...
  async function getRandomQueryFromHistory() {
    ...
    let randomQuery  = await response.json();
      ...
      let queryHTML = '';
      const requestDateString = new Date(randomQueryData.request_time).toLocaleString(); 
  ...
  function displayData(data) {
    ...
    data.forEach(item => {
      ...
      itemDiv.innerHTML = `
        <p><strong>Request Time:</strong> ${requestDate}, <strong>Query Type:</strong> ${item.queryType}</p>
  ...
  function filterData(data) {
    ...
    return data.filter(item => {
        ...
        const queryTypeValid = queryTypeFilter === 'all' || item.queryType === queryTypeFilter;
  ...
  function initializeFilters() {
    document.getElementById('queryTypeFilter').addEventListener('change', applyFilters);
  ...
  async function getStatistics() {
    try {
      ...
      if (data.userRequests && typeof data.userRequests === 'object' && 'count' in data.userRequests && 'requestTypes' in data.userRequests) {
        ...
        const result = `
          ...
          <h4>Request Types:</h4>
          <ul>
            ${userRequests.requestTypes.map(type => `<li>${type._id}: ${type.count}</li>`).join('')}
          </ul>
        `;
        document.getElementById('statistics-result').innerHTML = result;
      } else {
        console.error('Error: Invalid data structure', data.userRequests);
        document.getElementById('statistics-result').innerHTML = '<p>Error: Invalid data structure.</p>';
      }
    } catch (error) {
      console.error('Error:', error);
      document.getElementById('statistics-result').innerHTML = 'Error fetching statistics from server.';
    }
  }
